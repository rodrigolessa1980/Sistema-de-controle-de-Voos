...
export async function tripRoutes(app: FastifyInstance): Promise<void> {
  ...
  route.post(
    ...
    async (request, reply) => {
      ...
      // O admin pode lançar voos já realizados (histórico), então para ele a
      // data no passado é permitida.
...
export async function createPassengers(
  ...
  await tx.passenger.createMany({
    data: pax.map((p, index) => ({
      // Nome é opcional; em branco, identifica pela posição.
      name: p.name === undefined || p.name === '' ? `Passageiro ${index + 1}` : p.name,
      position: index,
